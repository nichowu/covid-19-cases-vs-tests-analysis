...
main <- function(input_path, out_dir) {
  ...
  dist_can <-
    create_distribution_plot(covid_can_usa, "CAN", "Canada")
  
  dist_usa <-
    create_distribution_plot(covid_can_usa, "USA", "USA")
  
  histo_plot <- plot_grid(dist_usa, dist_can, ncol = 1)
  
  #save histogram as a .png file
  ggsave(
    paste0(out_dir, "/histogram_plot.png"),
    histo_plot,
    width = 5,
    height = 3
  )
  ...
  ggsave(
    paste0(out_dir, "/violin_plot.png"),
    violin_plot,
    width = 8,
    height = 10
  )
